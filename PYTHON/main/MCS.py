...
def bootstrap_sample(data, B=500, w=5):
    """
    ...
    select = np.asfortranarray(np.random.rand(B, t).T < p)
    vals = np.ceil(np.random.rand(1, np.sum(np.sum(select))) * t).astype(int)
...
def calculate_PvalR(z, included, zdata0):
    ...
    zdata = zdata0[numpy.ix_(included - 1, included - 1)]
    TR = np.max(zdata)
...
def iterate(dijbar, dijbarstar, vardijbar, alpha, algorithm="R"):
    ...
    for i in range(M0 - 1):
        ...
        m = len(included)
        z = z0[numpy.ix_(range(B), included - 1, included - 1)]
        ...
        scale = m / (m - 1)
        dibar = np.mean(dijbar[numpy.ix_(included - 1, included - 1)], 0) * scale
        dibstar = np.mean(dijbarstar[numpy.ix_(range(B), included - 1, included - 1)], 1) * (
            m / (m - 1)
        )
