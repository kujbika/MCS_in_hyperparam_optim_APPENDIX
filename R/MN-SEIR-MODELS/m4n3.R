...
mn_optim <- function(parameter_values) {
  ...
  mn_seir_equations <- function(time, variables, parameters) {
    ...
    with(as.list(c(variables, parameters)), {
      if (time <= tau_q+1){
        ...
        lagged = deSolve :: lagvalue(time - tau_q)[c(1, 13)]
      }
      ...
      return(list(c(dS, dS_Q, dE1, dE2, dE3, dE4, dI_A1,dI_A2,dI_A3, dI_S1,dI_S2,dI_S3, dI_S, dI_A, dI, dP_I1, dP_I2, dP_I3, dQ, dR)))
    })
  }
  initial_values <- c(
    S=762,
  ...
  out = deSolve :: dede(
    ...
    parms = c(parameter_values, N=763, m=4, n=3, tau_q=5),
    method = "impAdams",
