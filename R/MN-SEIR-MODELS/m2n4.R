...
mn_optim <- function(parameter_values) {
  ...
  mn_seir_equations <- function(time, variables, parameters) {
    ...
    with(as.list(c(variables, parameters)), {
      ...
      dK <- -lambda * K
      return(list(c(dS,dI_S, dI, dS_Q, dE1, dE2, dI_A1,dI_A2,dI_A3,dI_A4, dI_S1,dI_S2,dI_S3,dI_S4, dI_A, dP_I1, dP_I2, dP_I3, dP_I4, dQ, dR, dK)))
    })
  }
  initial_values <- c(
    S=19449999,
  ...
  out = deSolve :: dede(
    ...
    parms = c(parameter_values, N=19450000, m=2, n=4, tau_q=14, d_I=1),
    method = "impAdams",
...
mn_pred <- function(parameter_values) {
  ...
  times=1:length(cases),
  func=mn_seir_equations,
  parms=c(parameter_values, N=19450000, m=2, n=4, tau_q=14),
  method="impAdams",
  control=list(interpol=2)
  )
